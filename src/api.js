...
// Tipically API calls includes some of the query params:
//  - "q" for search
//  - "limit" for limiting the amount of results
//  - "sort" for specifying the order of the results
// We emulate an async call, returning an array of sorted results
export const getEmailList = (searchTerm, limit) => new Promise((resolve) => {
  const apiResponse = emails
    // filtering by searchTerm param
    .filter(e => e.includes(searchTerm))
    // sorting by indexOf the indexOf of searchTerm occurrence, then alphabetically
    .sort((a, b) => {
